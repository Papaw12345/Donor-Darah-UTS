...
#### Keputusan Proyek Phase 8L - Pemberitahuan Petugas

Rincian berikut mengunci pembuatan pemberitahuan in-app oleh Petugas untuk satu kandidat Pendonor dari alur Pemanggilan Pendonor.

- Phase 8L hanya membuat satu row `pemberitahuan` untuk satu kandidat Pendonor yang dipilih dari konteks persediaan rendah. Pembuatan row tersebut merupakan arti "mengirim" di dalam prototype. Phase ini bukan SMS, WhatsApp, email, realtime/WebSocket push, Laravel Notification infrastructure, background job, queue, external delivery tracking, patient matching, crossmatch, transfusion compatibility, hospital request, atau clinical decision support.
- Fitur hanya tersedia bagi akun terautentikasi dengan `status_akun = AKTIF`, `peran = PETUGAS`, dan relasi profil `petugas` yang valid. Authorization tetap server-side. Pengirim selalu Petugas yang sedang terautentikasi; nilai client tidak boleh menentukan atau menimpa `id_petugas_pengirim`, identitas pengirim, `waktu_dibuat`, maupun `waktu_dibaca`.
- Phase 8K tetap menjadi sumber identifikasi kandidat dan GET-nya tetap read-only. Setiap kandidat yang masih valid pada konteks low-stock terpilih dapat mempunyai aksi nyata `Buat Pemberitahuan` menuju form Phase 8L. Tidak ada state Pendonor terpilih yang disimpan pada session, tabel, field, atau entitas sementara.
- Phase 8L mengunci tepat dua route: `GET /petugas/pemanggilan/{ambang}/pendonor/{pendonor}/pemberitahuan` bernama `petugas.pemberitahuan.create` untuk menampilkan form, dan `POST` pada URI yang sama bernama `petugas.pemberitahuan.store` untuk memvalidasi state saat ini lalu membuat satu row. Keduanya berada di bawah middleware `web`, `auth`, `active`, dan `role:PETUGAS`. Tidak ada route `PATCH`, `PUT`, `DELETE`, bulk-send, atau CRUD manajemen pemberitahuan Petugas.
- Resource `{ambang}` dan `{pendonor}` hanya mengidentifikasi konteks dan target yang diminta. GET dan POST harus menghitung serta memvalidasi ulang authority, low-stock, dan kandidat di server. Hidden field atau request value tidak menjadi authority untuk `id_ambang`, `id_pendonor`, `id_petugas_pengirim`, golongan darah, jumlah stok, eligibility historis, atau timestamp.
- Pada GET dan POST, row ambang existing harus tetap low-stock menurut semantik Phase 8J/8K: tanggal acuan adalah hari ini menurut `Asia/Jakarta`; hanya unit `TERSEDIA` dengan `tanggal_kedaluwarsa >= tanggal_acuan` yang dihitung; grouping tepat pada `id_jenis_komponen` dan `id_golongan_darah`; stok tanpa unit eligible bernilai 0; dan kondisi rendah tepat ketika `jumlah_persediaan <= jumlah_minimum`. Tidak ada ambang default atau kombinasi tanpa konfigurasi yang disintesis.
- Resource route yang tidak ada menghasilkan HTTP 404. Jika row ambang existing tidak lagi low-stock, GET maupun POST menghasilkan HTTP 409 dengan pesan `Kondisi persediaan yang dipilih tidak sedang berada pada atau di bawah ambang.` dan tidak membuat pemberitahuan.
- Target Pendonor harus tetap ada, mempunyai `id_golongan_darah` yang sama tepat dengan ambang, terhubung ke akun `peran = PENDONOR` dan `status_akun = AKTIF`, serta memenuhi eligibility historis Phase 7H pada tanggal WIB saat request. Hanya penyumbangan `BERHASIL` sampai tanggal acuan yang dihitung; riwayat berhasil masa depan dan `GAGAL` diabaikan; interval memakai sukses terbaru ditambah 2 bulan kalender tanpa overflow; batas tahunan tetap 6 untuk `LAKI_LAKI` dan 4 untuk `PEREMPUAN`; dan Pendonor pertama kali dapat memenuhi kriteria. Hasil tetap eligibility historis, bukan kelayakan medis akhir.
- Jenis komponen hanya menjadi konteks persediaan rendah dan tidak menjadi aturan kemampuan medis Pendonor untuk menghasilkan WB/PRC/TC/FFP. Jika Pendonor existing tidak lagi valid, GET maupun POST menghasilkan HTTP 409 dengan pesan `Pendonor tidak lagi memenuhi kriteria pemanggilan untuk kondisi persediaan ini.` dan tidak membuat pemberitahuan.
- Form hanya boleh menampilkan informasi minimum untuk memastikan target: `nomor_donor`, `nama_lengkap`, ABO, Rhesus, konteks komponen/persediaan rendah, jumlah persediaan derived saat ini, dan `jumlah_minimum`. Form tidak menampilkan NIK, alamat lengkap, nomor telepon, email, tempat/tanggal lahir, pekerjaan, alamat kantor, password, atau data autentikasi.
- Satu-satunya input bisnis adalah `isi_pesan`. Nilai harus berupa string, di-trim oleh server, dan tidak boleh kosong setelah trim. Karena schema menggunakan `TEXT` dan belum mengunci batas bisnis, Phase 8L tidak menciptakan batas panjang arbitrer, title, subject, template entity, category, priority, channel, atau status.
- POST yang berhasil membuat tepat satu row existing-schema `pemberitahuan`: `id_pendonor` dari target yang divalidasi ulang server, `id_petugas_pengirim` dari profil Petugas terautentikasi, `isi_pesan` yang sudah divalidasi dan di-trim, `waktu_dibuat` dari waktu server sesuai konvensi aplikasi, serta `waktu_dibaca = NULL`. Hanya alur Pendonor Phase 7I yang mengelola `waktu_dibaca`.
- Setelah berhasil, alur kembali ke `petugas.pemanggilan.index?id_ambang=<selected existing threshold>` dengan flash tepat `Pemberitahuan berhasil dikirim kepada Pendonor.`
- Setiap POST valid dievaluasi secara independen. Pengiriman valid berulang boleh membuat row pemberitahuan baru karena schema dan requirement tidak menetapkan one-message-only. Tidak dibuat idempotency token, deduplication table, UNIQUE baru, field already-notified, session deduplication, atau lifecycle tersembunyi.
- Phase 8L tidak menambahkan halaman riwayat kirim, index/detail pemberitahuan Petugas, edit, delete, resend, draft, mark read/unread oleh Petugas, atau menu CRUD `pemberitahuan`. Dashboard Petugas juga tidak memperoleh menu terpisah `Pemberitahuan Petugas`; Phase 8L dicapai melalui aksi kandidat nyata pada Pemanggilan Pendonor.
- Perilaku receiver Phase 7I tetap: Pendonor hanya melihat miliknya sendiri; ordering tetap `waktu_dibuat DESC`, lalu `id_pemberitahuan DESC`; GET list/detail read-only; PATCH menandai dibaca; unread count berasal dari `waktu_dibaca IS NULL`; dan Dashboard Pendonor menampilkan maksimum 3 pesan terbaru. Row Phase 8L harus muncul secara alami melalui query tersebut tanpa mengubah ownership receiver.
- Mutation Phase 8L terbatas pada satu `INSERT` ke `pemberitahuan`. Tidak ada perubahan pada `ambang_persediaan`, `unit_komponen_darah`, `pendonor`, `akun`, `penyumbangan`, `seleksi_donor`, atau `pemesanan_donor`; tidak ada decrement stok, reservasi stok, atau reservasi kandidat. Transaction atau row lock tidak diwajibkan hanya untuk insert setelah revalidasi.
- Implementasi kelak tetap sederhana dan dapat dijelaskan dengan PK/FK, server-side authorization, `SELECT`/`JOIN`/aggregate untuk revalidasi, validation, `INSERT`, Eloquent/Query Builder, Laravel Controller, dan Blade. Phase 8L tidak menambahkan service/repository/DTO, event/listener, queue/job, notification framework/package, SPA/AJAX, WebSocket/realtime, search, pagination, atau provider pesan eksternal. Konsolidasi kalkulasi lintas controller dapat dievaluasi pada Phase 9.
- Schema tidak berubah: tetap 15 tabel bisnis dan 97 field. Phase 8L tidak menambahkan tabel, field, PK, FK, UNIQUE, custom index, enum, migration, timestamp field, soft delete, `remember_token`, maupun tabel `pemanggilan`, `candidate`, `notification_delivery`, `sent_notifications`, atau `message_template`.
