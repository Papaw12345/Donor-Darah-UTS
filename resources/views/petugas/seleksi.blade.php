...
@section('content')
    <div class="container page-shell">
        <header class="page-header"><div class="page-header-main"><h1 class="page-title">Seleksi Donor</h1></div><a class="button button-secondary" href="{{ route('petugas.kuesioner.show', $pemesanan) }}">Kembali ke Kuesioner</a></header>
        @include('partials.alerts')

        {{-- Data pemesanan --}}
        <section class="page-section" aria-labelledby="konteks-kunjungan">
            <h2 class="section-title" id="konteks-kunjungan">Data Kunjungan</h2>
            <dl class="identity-panel identity-grid">
                ...
                <div class="identity-item"><dt>Waktu Jadwal</dt><dd>{{ $pemesanan->jadwalPelayanan->jam_mulai }} - {{ $pemesanan->jadwalPelayanan->jam_selesai }}</dd></div><div class="identity-item"><dt>Status Jadwal</dt><dd>{{ $pemesanan->jadwalPelayanan->status_jadwal }}</dd></div>
                <div class="identity-item"><dt>Waktu Check-in</dt><dd>{{ $pemesanan->waktu_checkin?->format('Y-m-d H:i:s') ?? 'Belum tersedia' }}</dd></div>
                <div class="identity-item"><dt>Golongan Darah</dt><dd>@if ($pemesanan->pendonor->golonganDarah !== null){{ $pemesanan->pendonor->golonganDarah->abo }} {{ $pemesanan->pendonor->golonganDarah->rhesus }}@else Belum terkonfirmasi @endif</dd></div>
                <div class="identity-item"><dt>Status Pemesanan</dt><dd><span class="status-badge status-success">{{ ['TERJADWAL' => 'Terjadwal', 'CHECK_IN' => 'Check-in', 'SELESAI' => 'Selesai', 'TIDAK_HADIR' => 'Tidak Hadir'][$pemesanan->status_pemesanan] ?? $pemesanan->status_pemesanan }}</span></dd></div>
            </dl>
        </section>
        ...
        @if ($seleksi === null)
            ...
            <section class="page-section" aria-labelledby="form-seleksi">
                <h2 class="section-title" id="form-seleksi">Form Seleksi Donor</h2>
        ...
        @else
            <section class="page-section" aria-labelledby="seleksi-tersimpan">
                <h2 class="section-title" id="seleksi-tersimpan">Seleksi Tersimpan</h2>
                <dl class="identity-panel identity-grid">
                    <div class="identity-item"><dt>Berat Badan</dt><dd>{{ $seleksi->berat_badan }}</dd></div><div class="identity-item"><dt>Tekanan Sistolik</dt><dd>{{ $seleksi->tekanan_sistolik }}</dd></div>
                    <div class="identity-item"><dt>Tekanan Diastolik</dt><dd>{{ $seleksi->tekanan_diastolik }}</dd></div><div class="identity-item"><dt>Denyut Nadi</dt><dd>{{ $seleksi->denyut_nadi }}</dd></div>
                    <div class="identity-item"><dt>Suhu Tubuh</dt><dd>{{ $seleksi->suhu_tubuh }}</dd></div><div class="identity-item"><dt>Kadar Hb</dt><dd>{{ $seleksi->kadar_hb }}</dd></div>
                    <div class="identity-item"><dt>Hasil Pemeriksaan Kesehatan</dt><dd>{{ $seleksi->hasil_pemeriksaan_kesehatan ?? 'Tidak ada' }}</dd></div><div class="identity-item"><dt>Keputusan Seleksi</dt><dd><span class="status-badge {{ $seleksi->keputusan_seleksi === 'LAYAK' ? 'status-success' : ($seleksi->keputusan_seleksi === 'DITOLAK' ? 'status-danger' : 'status-warning') }}">{{ $seleksi->keputusan_seleksi }}</span></dd></div>
                    <div class="identity-item"><dt>Alasan Keputusan</dt><dd>{{ $seleksi->alasan_keputusan ?? 'Tidak ada' }}</dd></div><div class="identity-item"><dt>Petugas Pencatat</dt><dd>@if ($seleksi->petugas !== null){{ $seleksi->petugas->nama_petugas }} ({{ $seleksi->petugas->nomor_petugas }})@else Tidak tersedia @endif</dd></div>
                    <div class="identity-item"><dt>Waktu Seleksi</dt><dd>{{ $seleksi->waktu_seleksi->format('Y-m-d H:i:s') }}</dd></div>
                </dl>
